...
/**
 * @brief 执行DAP命令,处理请求并准备响应
 * 
 * @details 该函数处理两种情况:
 *          1. 单个DAP命令: 直接调用DAP_ProcessCommand处理
 *          2. 多个DAP命令(ID_DAP_ExecuteCommands): 循环处理每个子命令
 *
 * @param request  指向请求数据的指针,包含要执行的DAP命令
 * @param response 指向响应数据的指针,用于存储命令执行结果
 * @return uint32_t 返回32位值:
 *                  - 低16位: 响应数据的字节数
 *                  - 高16位: 请求数据的字节数
 */
uint32_t DAP_ExecuteCommand(const uint8_t *request, uint8_t *response) {
  uint32_t cnt;    // 命令计数器
  uint32_t num;    // 总字节数计数器
  uint32_t n;      // 单个命令处理的字节数
  ...
  if (*request == ID_DAP_ExecuteCommands) {
    ...
    *response++ = (uint8_t)cnt;
    ...
    while (cnt--) {
      ...
      /* 更新请求和响应指针位置 */
      request  += (uint16_t)(n >> 16);    // 移动到下一个请求
      response += (uint16_t) n;           // 移动到下一个响应位置
    }
    return (num);
  }
...
/**
 * @brief DAP初始化设置函数
 * @note 用于初始化DAP相关参数和配置
 */
void DAP_Setup(void) {

  /* 默认设置初始化 */
  DAP_Data.debug_port  = 0U;                        // 调试端口设为0
  DAP_Data.nominal_clock = DAP_DEFAULT_SWJ_CLOCK;   // 设置默认SWJ时钟
  DAP_Data.transfer.idle_cycles = 0U;               // 空闲周期设为0
  DAP_Data.transfer.retry_count = 100U;             // 重试次数设为100
  DAP_Data.transfer.match_retry = 0U;               // 匹配重试设为0
  DAP_Data.transfer.match_mask  = 0x00000000U;      // 匹配掩码设为0

#if (DAP_SWD != 0)
  /* SWD模式相关配置 */
  DAP_Data.swd_conf.turnaround  = 1U;              // 转向延时设为1
  DAP_Data.swd_conf.data_phase  = 0U;              // 数据相位设为0
#endif

#if (DAP_JTAG != 0)
  /* JTAG模式相关配置 */
  DAP_Data.jtag_dev.count = 0U;                    // JTAG设备数量设为0
#endif
